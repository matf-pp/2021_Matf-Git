    ...
    where fja acc fp
            ...
            | otherwise           = acc
                where newR  = newRemove acc fp
                      ...
                      pom (old,off) (_,(Remove ind br)) = (dropBetween old (ind + off - 1) br, off - br)
                      pom (old,off) (True,(Add ind s)) = (insertBetween s old $ ind - 1, off + length s)
                      pom (old,off) (False,(Add ind s))     = (insertBetween s old $ ind + off, off + length s)
